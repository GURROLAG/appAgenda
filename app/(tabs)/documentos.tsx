  ...
  return (
    <TouchableOpacity
      ...
      <View style={{ flexDirection: 'row', alignItems: 'flex-start', marginBottom: 8 }}>
        ...
        <View style={{ flex: 1 }}>
          <Text style={{ fontWeight: 'bold', fontSize: 16, color: colors.text }}>{nota.titulo}</Text>
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6, marginTop: 4, flexWrap: 'wrap' }}>
            <View style={{ backgroundColor: cat.color + '22', paddingHorizontal: 8, paddingVertical: 2, borderRadius: 10 }}>
              <Text style={{ color: cat.color, fontSize: 11, fontWeight: 'bold' }}>{cat.label}</Text>
            </View>
            <Text style={{ color: colors.subtext, fontSize: 12 }}>📅 {nota.fechaManual}</Text>
            {nota.tipo !== 'privado' && (
              <Text style={{ color: colors.subtext, fontSize: 12 }}>👤 {nota.nombreAutor}</Text>
            )}
            {/* Badge tipo */}
            {nota.tipo === 'compartido' && (
              <View style={{ backgroundColor: '#fff3e0', paddingHorizontal: 8, paddingVertical: 2, borderRadius: 10 }}>
                <Text style={{ color: '#f59e0b', fontSize: 11, fontWeight: 'bold' }}>👥 Compartido</Text>
              </View>
            )}
          </View>
        </View>
      ...
      {/* Contenido */}
      {nota.contenido ? (
        <Text style={{ color: colors.subtext, fontSize: 14, lineHeight: 20, marginBottom: 6 }}>
          {nota.contenido}
        </Text>
      ) : null}
        ...
        <View style={{ marginTop: 8, gap: 6 }}>
          {nombresCompartidos.map((nombre, idx) => (
            <View key={idx} style={{ flexDirection: 'row', alignItems: 'center', gap: 8, backgroundColor: colors.input, padding: 8, borderRadius: 10 }}>
              <View style={{ width: 28, height: 28, borderRadius: 14, backgroundColor: colors.primary + '22', justifyContent: 'center', alignItems: 'center' }}>
      ...
      {nota.tipo === 'compartido' && !esPropia && (
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6, marginTop: 10, paddingTop: 10, borderTopWidth: 1, borderTopColor: colors.border }}>
          ...
          <Text style={{ color: colors.subtext, fontSize: 12 }}>Compartido contigo por {nota.nombreAutor}</Text>
        </View>
      )}
    </TouchableOpacity>
  );
...
export default function DocumentosScreen() {
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: colors.background }} edges={['top']}>
      ...
      {/* TABS */}
      <View style={{ flexDirection: 'row', marginHorizontal: 16, marginBottom: 12, backgroundColor: colors.input, borderRadius: 14, padding: 4 }}>
        {(['privado', 'compartido', 'general'] as TabActivo[]).map((tab) => {
          ...
          return (
            ...
            >
              ...
              <Text style={{ fontWeight: activo ? 'bold' : 'normal', color: activo ? colors.primary : colors.subtext, fontSize: 13 }}>{label}</Text>
      ...
      {/* BUSCADOR */}
      <View style={{ flexDirection: 'row', alignItems: 'center', marginHorizontal: 16, marginBottom: 12, backgroundColor: colors.input, borderRadius: 12, paddingHorizontal: 14 }}>
        ...
        <TextInput
          ...
          style={{ flex: 1, paddingVertical: 12, paddingLeft: 10, color: colors.text, fontSize: 15 }}
        />
        ...
        <ScrollView contentContainerStyle={{ paddingHorizontal: 16, paddingBottom: 100 }}>
          {notasFiltradas.length === 0 ? (
            <View style={{ alignItems: 'center', marginTop: 60 }}>
              ...
              <Text style={{ color: colors.subtext, fontSize: 16, marginTop: 16, fontWeight: 'bold' }}>
                {busqueda ? 'Sin resultados'
                  : tabActivo === 'privado' ? 'No tienes notas privadas'
                  : tabActivo === 'compartido' ? 'Sin notas compartidas'
                  : 'No hay notas generales'}
              </Text>
              <Text style={{ color: colors.subtext, fontSize: 14, marginTop: 6, textAlign: 'center' }}>
        ...
        <View style={{ flex: 1, justifyContent: 'flex-end', backgroundColor: '#00000088' }}>
          ...
          {notaDetalle && (() => {
            ...
            const nombresComp = (notaDetalle.compartidoCon || [])
              .map((uid) => todosUsuarios.find((u) => u.uid === uid)?.nombre ?? 'Usuario');
            return (
              <View style={{ backgroundColor: colors.card, borderTopLeftRadius: 25, borderTopRightRadius: 25, paddingHorizontal: 24, paddingTop: 16, paddingBottom: 40, maxHeight: '90%' }}>
                {/* Handle */}
                ...
                <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 20 }}>
                  ...
                  <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 12, flexWrap: 'wrap' }}>
                    <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6, backgroundColor: cat.color + '22', paddingHorizontal: 12, paddingVertical: 6, borderRadius: 20 }}>
                      <Ionicons name={cat.icono as any} size={16} color={cat.color} />
                      <Text style={{ color: cat.color, fontWeight: 'bold', fontSize: 13 }}>{cat.label}</Text>
                    </View>
                    <View style={{ backgroundColor: colors.input, paddingHorizontal: 10, paddingVertical: 6, borderRadius: 20 }}>
                      <Text style={{ color: colors.subtext, fontSize: 12 }}>📅 {notaDetalle.fechaManual}</Text>
                    </View>
                    {notaDetalle.tipo === 'compartido' && (
                      <View style={{ backgroundColor: '#fff3e0', paddingHorizontal: 10, paddingVertical: 6, borderRadius: 20 }}>
                        <Text style={{ color: '#f59e0b', fontSize: 12, fontWeight: 'bold' }}>👥 Compartido</Text>
                      </View>
                    )}
                    {notaDetalle.tipo === 'general' && (
                      <View style={{ backgroundColor: '#f0fff0', paddingHorizontal: 10, paddingVertical: 6, borderRadius: 20 }}>
                        <Text style={{ color: '#32cd32', fontSize: 12, fontWeight: 'bold' }}>🌐 General</Text>
                      </View>
                    )}
                    {notaDetalle.tipo === 'privado' && (
                      <View style={{ backgroundColor: '#e8f4ff', paddingHorizontal: 10, paddingVertical: 6, borderRadius: 20 }}>
                        <Text style={{ color: colors.primary, fontSize: 12, fontWeight: 'bold' }}>🔒 Privado</Text>
                      </View>
                    )}
                  </View>
                  ...
                  {notaDetalle.contenido ? (
                    ...
                    <Text style={{ color: colors.subtext, fontSize: 15, fontStyle: 'italic', marginBottom: 20 }}>
                  ...
                  {notaDetalle.tipo === 'compartido' && nombresComp.length > 0 && (
                    <View style={{ backgroundColor: colors.input, borderRadius: 14, padding: 14, marginBottom: 20 }}>
                      <Text style={{ color: colors.subtext, fontSize: 13, fontWeight: 'bold', marginBottom: 10 }}>
                      ...
                      <View style={{ gap: 8 }}>
                        {nombresComp.map((nombre, idx) => (
                          <View key={idx} style={{ flexDirection: 'row', alignItems: 'center', gap: 10 }}>
                            <View style={{ width: 32, height: 32, borderRadius: 16, backgroundColor: colors.primary + '22', justifyContent: 'center', alignItems: 'center' }}>
                              <Ionicons name="person" size={16} color={colors.primary} />
                            </View>
                            <Text style={{ color: colors.text, fontSize: 14 }}>{nombre}</Text>
                          </View>
                        ))}
                      </View>
                    </View>
                  ...
                  {notaDetalle.creadoPor === usuario?.uid && (
                    <View style={{ flexDirection: 'row', gap: 12 }}>
                      <Pressable
                        ...
                        style={{ flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, paddingVertical: 14, borderRadius: 14, backgroundColor: colors.input }}
                      >
                        <Ionicons name="pencil-outline" size={18} color={colors.text} />
                        <Text style={{ color: colors.text, fontWeight: 'bold', fontSize: 15 }}>Editar</Text>
                      </Pressable>
                      <Pressable
                        onPress={() => { setNotaDetalle(null); setTimeout(() => setNotaAEliminar(notaDetalle), 300); }}
                        style={{ flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, paddingVertical: 14, borderRadius: 14, backgroundColor: '#fff0f0' }}
                      >
                        <Ionicons name="trash-outline" size={18} color="#ef4444" />
                        <Text style={{ color: '#ef4444', fontWeight: 'bold', fontSize: 15 }}>Eliminar</Text>
                      </Pressable>
                    </View>
                  )}
                </ScrollView>
      ...
      <Modal visible={modalVisible} transparent animationType="slide">
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={{ flex: 1 }}>
          <View style={{ flex: 1, justifyContent: 'flex-end', backgroundColor: '#00000088' }}>
            <View style={{ backgroundColor: colors.card, borderTopLeftRadius: 25, borderTopRightRadius: 25, paddingHorizontal: 24, paddingTop: 16, paddingBottom: 40, maxHeight: '94%' }}>
              <View style={{ width: 40, height: 4, borderRadius: 2, backgroundColor: colors.border, alignSelf: 'center', marginBottom: 16 }} />
              ...
              <ScrollView showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled" contentContainerStyle={{ paddingBottom: 16 }}>
                ...
                <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
                  ...
                  <View style={{
                    ...
                    <Text style={{ color: tabActivo === 'privado' ? colors.primary : tabActivo === 'compartido' ? '#f59e0b' : '#32cd32', fontSize: 12, fontWeight: 'bold' }}>
                ...
                {/* Título */}
                <Text style={{ color: colors.subtext, fontSize: 13, marginBottom: 8 }}>Título *</Text>
                <TextInput
                ...
                {/* Categoría */}
                <Text style={{ color: colors.subtext, fontSize: 13, marginBottom: 10 }}>Categoría</Text>
                    ...
                    {CATEGORIAS.map((cat) => {
                      ...
                      return (
                        <TouchableOpacity
                          ...
                          style={{ flexDirection: 'row', alignItems: 'center', gap: 6, paddingHorizontal: 14, paddingVertical: 10, borderRadius: 20, borderWidth: 1.5, borderColor: activa ? cat.color : colors.border, backgroundColor: activa ? cat.color + '22' : colors.input }}
                        >
                          <Ionicons name={cat.icono as any} size={15} color={activa ? cat.color : colors.subtext} />
                          <Text style={{ color: activa ? cat.color : colors.subtext, fontSize: 13, fontWeight: activa ? 'bold' : 'normal' }}>{cat.label}</Text>
                        </TouchableOpacity>
                      );
                    })}
                  </View>
                ...
                {/* Fecha */}
                <Text style={{ color: colors.subtext, fontSize: 13, marginBottom: 8 }}>Fecha</Text>
                <TextInput
                ...
                {/* Contenido */}
                <Text style={{ color: colors.subtext, fontSize: 13, marginBottom: 8 }}>Contenido</Text>
                <TextInput
                ...
                {tabActivo === 'compartido' && (
                  <View style={{ marginBottom: 20, backgroundColor: colors.input, borderRadius: 16, padding: 16 }}>
                    ...
                    {todosUsuarios.length === 0 ? (
                      <Text style={{ color: colors.subtext, fontSize: 13, textAlign: 'center' }}>No hay otros usuarios registrados</Text>
                    ) : (
                      <View style={{ gap: 8 }}>
                        {todosUsuarios.map((u) => {
                          ...
                          return (
                            <TouchableOpacity
                              ...
                              style={{ flexDirection: 'row', alignItems: 'center', gap: 12, padding: 12, borderRadius: 12, borderWidth: 1.5, borderColor: sel ? colors.primary : colors.border, backgroundColor: sel ? colors.primary + '12' : colors.card }}
                            >
                              ...
                              <Text style={{ flex: 1, color: colors.text, fontSize: 15, fontWeight: sel ? 'bold' : 'normal' }}>{u.nombre}</Text>
                              <View style={{ width: 24, height: 24, borderRadius: 12, borderWidth: 2, borderColor: sel ? colors.primary : colors.border, backgroundColor: sel ? colors.primary : 'transparent', justifyContent: 'center', alignItems: 'center' }}>
                ...
                {/* Botones guardar */}
                <View style={{ flexDirection: 'row', gap: 12 }}>
                  <Pressable onPress={() => setModalVisible(false)} style={{ flex: 1, paddingVertical: 16, borderRadius: 14, alignItems: 'center', backgroundColor: colors.input }}>
                    <Text style={{ color: colors.text, fontWeight: 'bold', fontSize: 16 }}>Cancelar</Text>
                  </Pressable>
                  <Pressable onPress={guardar} disabled={guardando} style={{ flex: 1, paddingVertical: 16, borderRadius: 14, alignItems: 'center', backgroundColor: colors.primary }}>
                    <Text style={{ color: '#fff', fontWeight: 'bold', fontSize: 16 }}>{guardando ? 'Guardando...' : 'Guardar'}</Text>
                  </Pressable>
                </View>
